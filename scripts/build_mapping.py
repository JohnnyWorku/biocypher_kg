...
def create_sample(file_path, output_dir, sample_root, fraction=DEFAULT_SAMPLE_FRACTION):
    ...
    if total_lines <= min_lines_for_sampling:
        ...
        logger.info(f"Sample created (full copy — file too small to sample): {sample_path} ({total_lines} lines)")
        return
...
def download_one(url, output_path, extract, compress=None, move_to_dest=None,
    ...
    # Use the same size-comparison logic as already_downloaded to detect stale files.
    ...
    # For files moved outside output_dir, use the destination directory as the
    # sample anchor so the sample mirrors the same relative structure there.
    if output_dir and sample_root:
        if output_path.exists():
            if move_to_dest:
                # File is outside output_dir — place sample inside move_to_dest/sample/
                moved_sample_root = move_to_dest / 'sample'
                create_sample(output_path, move_to_dest, moved_sample_root, sample_fraction)
            else:
    ...
    if isinstance(urls, dict):
        if is_filename_url_dict(urls):
            ...
            for filename, url in urls.items():
                # Re-create sub_dir each iteration: a previous move_to may have deleted it
                sub_dir.mkdir(parents=True, exist_ok=True)
...
def ensure_samples(output_dir, sample_root, sample_fraction=DEFAULT_SAMPLE_FRACTION):
    """Create missing samples for all files already present in output_dir.

    Walks every file under output_dir (excluding the sample/ subdirectory itself)
    and calls create_sample for any file whose sample doesn't exist yet.
    This covers files that were skipped on the current run (already downloaded)
    as well as files downloaded in previous runs before sampling was introduced.
    """
    if not sample_root:
        return

    logger.info("Checking for missing samples...")
    created = skipped = 0

    for file_path in sorted(output_dir.rglob('*')):
        # Skip directories, and anything inside the sample tree itself
        if not file_path.is_file():
            continue
        try:
            file_path.relative_to(sample_root)
            continue  # it's inside sample/ — skip
        except ValueError:
            pass
        ...
        create_sample(file_path, output_dir, sample_root, sample_fraction)
        created += 1

    logger.info(f"Sample backfill complete: {created} created, {skipped} already existed")
...
def download_data(
    ...
    for source_key, source_data in config.items():
        ...
        if not has_handled:
            logger.warning(f"No 'url', 'directories', or 'zip_extract' key found for source '{source_key}' — skipping")
    ...
    # Create samples for any already-existing files that were skipped this run
    if sample_root:
        ensure_samples(output_dir, sample_root, sample_fraction)
